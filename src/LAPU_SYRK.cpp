...
int LAPU::Dump_Syrk_SMachine(){
	...
	switch (Syrk_Current_State){
		...
		case Syrk_MAC_Flush0:
			cout<<" Syrk_MAC_Flush0"<<endl;

		break;
		case Syrk_MAC_Flush1:
			cout<<" Syrk_MAC_Flush1"<<endl;

		break;
		case Syrk_MAC_Flush2:
			cout<<" Syrk_MAC_Flush2"<<endl;
...
int LAPU::Syrk_Kernel(int Global_index, double **& A, bool init){

	int count = 0;
	Syrk_Current_State = Syrk_BC0;

	// Initial PE Memories for Testing
	if (init) {
		for (i=0; i<Size; i++){
			PE_Array[i][0].Initialize_Local_Mem_New(A,10,10,0, 'A');
		}
	}
	...
	while (1){
		switch (Syrk_Current_State){
			...
			case Syrk_MAC_BC:
				...
				if (Kc_Counter_Curr== (Kernel_Size -1)){
					Syrk_Next_State = Syrk_MAC_Flush0;
			...
			case Syrk_MAC_Flush0:
				Syrk_Next_State = Syrk_MAC_Flush1;

			break;
			case Syrk_MAC_Flush1:
				Syrk_Next_State = Syrk_MAC_Flush2;
		...
		for (i=0;i<Size;i++)
			for (j=0;j<Size;j++){
				PE_Array[i][j].Execute_Syrk(Global_index, Kc_Counter_Curr, Syrk_Current_State, Latency_Counter_Curr);
		...
		if (((Syrk_Current_State==Syrk_MAC_Flush2) && done) /*|| Ap_Counter_Curr*/ ){
			 return Cycles_Passed;
		}
		Dump_Row_Buses();
		Dump_Column_Buses();
