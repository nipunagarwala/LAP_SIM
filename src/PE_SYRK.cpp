...
/* TODO: Create Recursive address generation for SYRK
 
*/
int PE::Recurs_Gen_A_Syrk(int Prev){
  
  // if (Prev==0) return 1;
  // else return (Recurs_Gen_A_Syrk(Prev-1) +1)%(Mem_Size_A);
	return (Prev+1);
...
int PE::Execute_Syrk(int Global_index, int Kc, int Syrk_Current_State, int Latency_Counter) {
	switch(Syrk_Current_State){
		case Syrk_BC0:
      		Address_A_New = 0;
      		if (My_Column==0) {
				Write_My_Row_Reg_Next=Local_Mem.Reg_Read_New(Address_A_New, toA);
				cout << "THIS IS THE VALUE OF THE NEXT REGGGGG: " << Write_My_Row_Reg_Next << endl;
			}
		  	//Address_A_New++;
    		Address_A_New = Recurs_Gen_A_Syrk(Address_A_New);
    	break;
    	...
    	case Syrk_BC1:
    		if (My_Column==0) {
				*Write_My_Row_Bus=Write_My_Row_Reg_Curr;
				Write_My_Row_Reg_Next=Local_Mem.Reg_Read_New(Address_A_New, toA);
				cout << "THIS IS THE VALUE OF THE NEXT REGGGGG: " << Write_My_Row_Reg_Next << endl;
				cout << "THIS IS THE VALUE OF THE BUSSSSS: " << *Write_My_Row_Bus << endl;
    		}
    	...
    	case Syrk_MAC_BC:
    		if (Kc==0){ // Loading accumulator takes a cycle, and we start new panel with kc=1;
				//cout<<"LOADING ACC"<<Scratch_Regs_Curr[1]<<endl;
				ALU.Execute_MAC( Scratch_Regs_Curr[0], (*Read_My_Col_Bus));
