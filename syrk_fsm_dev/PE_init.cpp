...
//This is recursive function for Address A
...
int PE::Execute_Matmul (int Global_index, int Bpj_Counter, int Mc, int Kc, int Ap_Counter, int Matmul_Current_State,int Latency_Counter){
		...
		switch(Matmul_Current_State){
			...
			case 2://Fetch A
				...
				// KC should go from 0 to Kernel_Size + LAPU_Size (For blocks of C)
					...
					if (Kc % Kernel_Size == 0){ //end of a row/beginning of new row
						...
						if (My_Row == ((Mc + LAPU_Size-1) % LAPU_Size)){ //
	              			...
	              			Local_Mem.Reg_Write_New(Address_WA_New,(*Read_My_Col_Bus), toA); //n elements pulld to A
	              			Address_WA_New = Recurs_Gen_A(Address_WA_New); //generate new address
	...
	/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
	}	else if (Global_index == 1) { //CASE FOR SYRK#######################################################################
		...
		switch(Matmul_Current_State){
			...
			case 1: //Fetch first panel of A 

				cout << "A0 State................" << endl;
				if (Kc==0) {
					if (My_Row==(LAPU_Size-1)){
						ALU.Load_Accumulator(*Read_My_Col_Bus); // load the target matrix into the accumulator
						Scratch_Regs_Next[1]=*Read_My_Col_Bus;  // this branch probably unnecessary for A1 - A3 right?
		       	 		Address_WB_New = 0;
					}
				}
			...
			case 1: //Write first panel to register and fetch second panel of A to memB
			...
			case 1: //Broadcast, write second panel to reg, fetch third panel
		        ...
		        if (Kc==0) {
					if (My_Row == (LAPU_Size-1)){
						ALU.Load_Accumulator(*Read_My_Col_Bus); // third panel
						Scratch_Regs_Next[1]=*Read_My_Col_Bus;
		       	 		Address_WB_New = 0;
					}
				}
